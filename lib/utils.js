...
module.exports.saveEntities = function(entities, category, fn) {
  ...
  return async.concat(queries, module.exports.executeQuery, function(err, results) {
    ...
    if (results.length === 1) {
      ...
      if (validator.isUUID(opts.input)) {
        ...
        if (!opts.no_cache_update) {
          return module.exports.buildCache(opts, function(err) {
            assert(err);
            opts.no_cache_update = true;
            module.exports.searchEntity(opts, cb);
          });
        }
        if (opts.quiet) { return cb(null, results); }
...
module.exports.buildCache = function(opts, fn) {
  var type = opts.filters._type;
  if (!type || !opts.client) {
    return fn();
  }
  ...
  case 'images':
  case 'volumes':
    opts.client.get('/' + type).then(function(res) {
      module.exports.saveEntities(res.body[type], type, fn);
    });
    break;
  default:
    fn();
...
var panic = module.exports.panic = function(msg) {
  ...
  console.error('');
  console.error('          https://github.com/moul/scaleway-cli-node/issues');
  console.error('');
  process.exit(-1);
};
...
module.exports.findKeyInDeepObject = findKeyInDeepObject = function(obj, key) {
  if (_.has(obj, key)) {
    return obj[key];
  }
  return _.flatten(_.map(obj, function(v) {
    return typeof v == "object" ? findKeyInDeepObject(v, key) : [];
  }), true)[0];
};
