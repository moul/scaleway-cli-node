...
program
  ...
  .action(function(server, options) {
    ...
    utils.searchEntity({input: server, _type: 'servers', client: client}, function(err, entity) {
      utils.assert(err);
  ...
  .action(function(image, options) {
    ...
    Q.all(_.map(options.volume, function(volume) {
      ...
      function(results) {
        ...
        // Resolve bootscript
        utils.searchEntity({input: options.bootscript, _type: 'bootscripts', client: client}, function(err, bootscriptEntity) {
...
program
  ...
  .on('--help', function() {
    ...
    console.log("    $ scw exec $(scw start --wait $(scw create docker)) /bin/bash");
...
program
  ...
  .action(function(image, options) {
    var client = utils.newApi(options);
    utils.searchEntity({input: image, _type: 'images', client: client}, function(err, entity) {
      utils.assert(err);
...
program
  ...
  .action(function(servers, options) {
    ...
    utils.searchEntities({inputs: servers, filters: { _type: 'servers' }, any: true, client: client}, function(err, entities) {
      utils.assert(err);
...
program
  ...
  .action(function(images, options) {
    ...
    utils.searchEntities({inputs: images, filters: { _type: 'images' }, any: true, client: client}, function(err, entities) {
      utils.assert(err);
...
program
  ...
  .option('-s, --sync', 'DEPRECATED. see --wait')
  .option('-w, --wait', 'synchronous start. wait for SSH to be ready')
  .option('-T, --timeout <secs>', 'set all timeout values to secs')
  .option('--boot-timeout <secs>', 'set the boot timeout to secs')
  .option('--ssh-timeout <secs>', 'set the ssh timeout to secs')
  .action(function(servers, options) {
    ...
    if (options.sync) {
      console.log("The 'start --sync' option was renamed to 'start --wait'");
      utils.panic("Exiting.");
    }
...
program
  ...
  .action(function(snapshot, tagName, options) {
    ...
    utils.searchEntity({input: snapshot, _type: 'snapshots', client: client}, function(err, entity) {
      utils.assert(err);
    ...
    utils.searchEntity({input: server, _type: 'servers', client: client}, function(err, entity) {
      ...
      client.get('/servers/' + entity._id)
        .then(function(res) {
          ...
          utils.sshExec(ip, 'ps', {}, function(statusCode) {
            if (statusCode === 0) {
              console.log(server);
            }
            process.exit(statusCode);
