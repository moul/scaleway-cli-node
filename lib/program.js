    ...
    utils.searchEntity({input: server, _type: 'servers'}, function(err, entity) {
      ...
      client.get('/servers/' + entity._id)
        .then(function(res) {
          ...
          if (options.secure || client.config.exec_secure) {
  ...
  .action(function(server, options) {
    ...
    utils.searchEntity({input: server, _type: 'servers'}, function(err, entity) {
      client.delete('/servers/' + entity._id)
        .then(function(res) {
          utils.assert(res.statusCode !== 204, res);
        }).catch(utils.panic);
    });
  });
...
program
  ...
  .action(function(server, options) {
    ...
    utils.searchEntity({input: serverId, _type: 'servers'}, function(err, entity) {
      client.post('/servers/' + entity._id + '/action', {
        action: 'poweron'
      })
        .then(function() {
          ...
          if (options.sync) {
            var bootTimeout = utils.panicTimeout(options.bootTimeout, 'server state is ready');

            utils.waitForServerState(client, entity._id, 'running', function(err, server) {
              clearTimeout(bootTimeout);

              var sshTimeout = utils.panicTimeout(options.sshTimeout, 'ssh port is ready');
